...
| Aspire: containers up, API not there yet, stack "hangs" after the banner | **read the API's log, don't wait it out** (§2 Mode A) — the dashboard's `/consolelogs/resource/auditworthy-api`, or the newest `%TEMP%\aspire-dcp*\<guid>_err`. Then: someone removed `.WithEnvironment("ASPNETCORE_ENVIRONMENT", "Development")` from the API resource, so it starts in Production and `AddPlenipoPlatform()` throws on dev-auth (#54 — `AppHostCompositionTests` fails for this); a stale Postgres **data volume** initialized with a different password (`docker volume rm auditworthy-pg-data`; dev data is throwaway); or someone re-added `WaitFor` on the two **database** resources — wait for the postgres **server**, never the databases, or the wait is circular |
