...
//
// Auditworthy's own API port. Keep in step with RUNBOOK §2, auditworthy.http's @base, README.md and
...
var api = builder.AddProject<Projects.Auditworthy_Host>("auditworthy-api", launchProfileName: null)
    ...
    // IsExternal on endpoints that already exist, and this resource had none. Seen on a live host
    // 2026-09-08 (#79): the dashboard listed `auditworthy-api` as Running with an EMPTY URL column,
    // DCP had launched it with neither --urls nor a launch profile, and Kestrel fell back to the
    // ASP.NET default, `Now listening on: http://localhost:5000` — a port Aspire knows nothing
    // about, absent from the dashboard, and the port RUNBOOK §2 told the reader to go and find
    // there. AppHostCompositionTests.Api_resource_declares_the_pinned_external_http_endpoint fails
    // the build if this declaration goes missing again.
    //
    // FIXED host port 9433, on the same reasoning as the Postgres pin above, and for one more:
    // several Plenipo products can run on one machine, and :5000 (and the RUNBOOK's Mode B :8094)
    // are the ports everything defaults to. An /alive probe against a shared port can answer 200
    // from a DIFFERENT product and read as a green verification of this one. 9433 is Auditworthy's
    // alone. Do NOT unpin this to dodge a bind conflict: the conflict is the guard working. Confirm
    // identity, never liveness alone — GET /api/platform/modules must list `compliance`.
    //
