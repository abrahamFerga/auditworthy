...
const guardAssertionDiff = join(scratch, 'guard-assertion.diff');
const ordinaryTestEditDiff = join(scratch, 'ordinary-test-edit.diff');
...
writeFileSync(guardAssertionDiff, [
  'diff --git a/tests/Auditworthy.IntegrationTests/ChatAndApprovalTests.cs b/tests/Auditworthy.IntegrationTests/ChatAndApprovalTests.cs',
  '--- a/tests/Auditworthy.IntegrationTests/ChatAndApprovalTests.cs',
  '+++ b/tests/Auditworthy.IntegrationTests/ChatAndApprovalTests.cs',
  '@@ -1,2 +1,1 @@',
  '-        Assert.Equal(HttpStatusCode.Forbidden, denied.StatusCode);',
  '         await Task.CompletedTask;',
  '',
].join('\n'));
writeFileSync(ordinaryTestEditDiff, [
  'diff --git a/tests/Auditworthy.Compliance.Tests/ControlMappingTests.cs b/tests/Auditworthy.Compliance.Tests/ControlMappingTests.cs',
  '--- a/tests/Auditworthy.Compliance.Tests/ControlMappingTests.cs',
  '+++ b/tests/Auditworthy.Compliance.Tests/ControlMappingTests.cs',
  '@@ -1,2 +1,2 @@',
...
const cases = [
  ...
  {
    name: 'guard-test assertion removal',
    result: () => run(guardAssertionDiff),
    status: 1,
    patterns: [/control_policy_locked/, /ChatAndApprovalTests\.cs/],
    why: 'PR #32 deleted two approval-lane assertions and the gate reported green, because a diff ' +
      'that only removes `Assert.` lines matches none of the CONTENT_RULES symbols. Silence is not a verdict.',
  },
  {
    name: 'ordinary assertion edit outside the guard list',
    result: () => run(ordinaryTestEditDiff),
    status: 0,
    patterns: [/not a loop branch: evidence gates skipped/],
    absent: [/control_policy_locked/],
    why: 'scoped to three named files on purpose — a gate that fires on every refactored assertion ' +
      'gets muted within a week, which is worse than the gap it closed',
  },
  {
